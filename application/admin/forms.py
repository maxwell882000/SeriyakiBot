...
class DishForm(FlaskForm):
    ...
    cooking_time = StringField('Время приготовления блюда', validators=[DataRequired('Введите время')])
    submit = SubmitField('Сохранить')
    ...
    def fill_from_object(self, dish: Dish):
        ...
        self.cooking_time.data = dish.cooking_time
    ...
    def validate_cooking_time(self, field):
        try:
            ...
            raise ValidationError('Укажите числовое значение времени приготовления')
        if int(field.data) <=0:
            raise ValidationError('Время приготовления не может быь отрицательным или равной нулю')
