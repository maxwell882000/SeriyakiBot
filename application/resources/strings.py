...
def from_dish(dish: Dish, language: str) -> str:
    ...
    if language == 'uz':
        if dish.description_uz:
            ...
            dish_content += '\n'
            dish_content += '<b>{}:</b> {}{}'.format(get_string('cooking_time', language), dish.cooking_time,
                                                     get_string('minut', language))
            dish_content += '\n'
        else:
            dish_content += '<b>{}</b>'.format(dish.name)
            dish_content += '\n\n'
...
def from_order(order: Order, language: str, total: int) -> str:
    ...
    for order_item in order.order_items.all():
        counter += 1
        time_max.append(order_item.dish.cooking_time or 0)
        ...
        order_item_str = order_item_tmpl.format(counter=counter,
                                                ...
                                                text=get_string('cooking_time', language),
                                                time=order_item.dish.cooking_time,
                                                minut=get_string('minut', language))
        order_content += order_item_str + '\n'
    if order.delivery_price:
        order_content += "<b>{}</b> {} {}\n\n{}: {}{}".format(get_string('cart.summary', language),
                                                   _format_number(total * currency_value + order.delivery_price),
                                                   get_string('sum', language),
                                                   get_string('total_cooking_time', language),
                                                   max(time_max),
                                                   get_string('minut', language))
    else:
        order_content += "<b>{}</b> {} {}\n\n{}: {}{}".format(get_string('cart.summary', language),
